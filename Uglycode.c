...
int test2(double px, double py, double m, double b) {
    if (py < m * px + b) {
        return -1; // point is under the line
    } else if (py == m * px + b) {
        return 0;
    } else {
        return 1; // over
...
// get input
    ...
    pointInPlane += scanf("%lf,%lf", &x0, &y0);
    printf ("Triangle Vertex  B (x,y): ");
    pointInPlane += scanf("%lf,%lf", &x1, &y1);
    printf ("Triangle Vertex  C (x,y): ");
    pointInPlane += scanf("%lf,%lf", &x2, &y2);
    printf ("Test Point (x,y): ");
    pointInPlane += scanf("%lf,%lf", &px, &py);
